...
export class FileDB<T extends Object> {
  ...
  public read(key: string | number): T | undefined {
    return this._load()[key];
  }

  public write(key: string | number, data: T) {
    this._load()[key] = data;
    this._modified = true;
  }
  ...
  public merge(key: string | number, data: Partial<T>, omit?: string[]) {
  ...
  public flush(force = false) {
    if (this._data && (force || this._modified)) {
      ...
      fs.writeFileSync(this._fn, JSON.stringify(envelope), { encoding: 'utf8' });
